        ...
        <form onSubmit={submitHandler} className="flex flex-col gap-5 mt-5 w-full">
         ...
         onChange={fileHandler } 
          ...
          value={formData.file}
            ...
            required
          />
          <button className="bg-orange-600 text-white px-6 py-2 w-full cursor-pointer rounded-md">Singup</button>
        </form>
        <p className="text-center mt-4">
          Already have an account? <Link to={"/login"} className="text-orange-600 cursor-pointer">Login Here</Link>
        </p>
      </div>
    </div>
  );
